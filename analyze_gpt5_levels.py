...
N_trials_per_problem_type = gpt5_results["low"]['num_trials']
...
plt.bar(x_points + (ind_bar_width * 1.5), gpt5_results["minimal"]['all_acc'], yerr=gpt5_results["minimal"]['all_err'], color=colors[3], edgecolor='black', width=ind_bar_width, ecolor='gray')
plt.ylim([0,1])
...
plt.legend([name.lower() for name in gpt5_results.keys()],fontsize=plot_fontsize,frameon=False, bbox_to_anchor=(1.1, 1))
hide_top_right(ax)
if args.interval_size == 1:
    plt.savefig('./gpt5_int1_' + alphabet + '_combined_results.png', dpi=300, bbox_inches="tight")
elif args.interval_size ==2:
    plt.savefig('./gpt5_int2_' + alphabet + '_combined_results.png', dpi=300, bbox_inches="tight")
plt.close()